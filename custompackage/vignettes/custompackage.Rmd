...
# Objectives
 ...
 The objective of this module is to describe how to use linear discriminant function analysis and provide a package containing functions to easily run LDAs and visualize output.
...
We will now walk through two examples. The first dataset has two natural grouping, while the second dataset has three.
...
Looking at the raw data, it is clear that variances are not equal among groups across the five behavioral variables. Group 2 has more variance generally. We can fix this by transforming our data. I plotted a log transformed version of these variables above and made new log transformed versions of each behavioral measure. We also want to make sure to limit multicollinearity in our data. We can see that there are a lot of correlations among the 5 variables. The worst of these are the Speed and Distance variables, which are almost perfectly correlated. The meandering variable is also highly correlated with the other variables. I removed Meandering and Distance and keep the other three variables (Speed, Mobility, Angular Speed).

To make sure we don't overstate the ability of our resulting function to predict grouping, we split the data into a training set and testing set. We also center and scale the data since our variables are measured with different units.
```{r}
library(caret)
